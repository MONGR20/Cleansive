...
- **1.6.42** - "Show only in combat" never had a native sound alert: out of combat the hidden grid emptied the registry, in combat the restriction guard refused to fill it. Sounds are now armed whenever the grid *would* show in combat, so they exist before the pull. Context exclusions (solo/group/raid), disabling, and hiding by hand still silence them. Also: a deferred pass no longer records a fake failure in the peak, and a deliberately empty registry no longer reports itself as active.
...
- **1.6.37** - a re-read of the restriction-mask code. The retry counter is now `styleRetry retried=N recovered=N` (it measures attempts, so it says so), a re-arm caused by a lifted restriction is no longer attributed to the player in the report, and one comment sat above the wrong function.
- **1.6.36** - the pending-style flag was a boolean, so the first combat exit under ChallengeMode consumed it and the real end of the key restyled nothing. The mark knew the restriction had lifted; nobody came back to ask. Fixes a defect introduced in 1.6.34.
