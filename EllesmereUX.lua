...
local function createPreview(parent)
    ...
    preview:SetPoint("TOPLEFT", 0, -412)
...
function NS:CreateOptions()
    ...
    self.optionChecks[#self.optionChecks + 1] = toggle(appearance, self.L.NAMES, 0, -28, 275, "showNames", function() self:RefreshAll(true) end, self.L.TIP_NAMES)
    ...
    resizeNote:SetPoint("TOPLEFT", 0, -286)
    resizeNote:SetWidth(560)
    resizeNote:SetJustifyH("LEFT")
    section(appearance, localized("Disposition", "Layout"), -318)
    local growLabel = text(appearance, self.L.GROW, 13, C.text)
    growLabel:SetPoint("TOPLEFT", 0, -348)
    local grow = button(appearance, "", 160, 28)
    grow:SetPoint("TOPLEFT", 150, -341)
    ...
    layoutModeTitle:SetPoint("TOPLEFT", 330, -348)
    local layoutMode = button(appearance, "", 150, 28)
    layoutMode:SetPoint("TOPLEFT", 425, -341)
    ...
    section(appearance, localized("Aperçu en direct", "Live preview"), -390)
    ...
    local explanation = text(dispels, localized("La couleur indique le clic à utiliser. Réorganise les types pour changer l'affectation.", "The color indicates which click to use. Reorder types to change the assignment."), 11, C.dim)
